...
A core requirement of this project is to ensure that all analysis, transformations, and visualizations can be reproduced by any user following a clearly documented workflow. This section provides an end-to-end description of the steps required to reproduce the results, from data acquisition to the generation of final datasets and plots. All steps reflect the final structure of the repository and the behavior of the `run_all.ipynb` automation notebook.

### Overview of Reproducible Components

The reproducible workflow consists of the following elements:

- Raw data acquisition from Kaggle (salary dataset must be downloaded manually).
- Cleaning and standardization of both datasets using Python and OpenRefine.
- Integration of the two datasets using standardized player names.
- Feature engineering to create derived metrics such as efficiency_index and value_index.
- Automated generation of final datasets and visualizations.
- Storage of output artifacts and logs in both the repository and a Box folder.
...
The performance dataset is licensed under CC BY 4.0 and is provided directly in the repository:

data/raw/performance_2022_2023.csv


No further action is needed for this dataset.

#### 1.2 Salary Dataset (Cannot Be Redistributed)

The salary dataset is governed by Kaggle’s "Other" license and cannot be stored or redistributed in this repository. Users must download it manually:

Kaggle link:  
https://www.kaggle.com/datasets/omarsobhy14/nba-players-salaries

After downloading, place the CSV file in the following directory:

data/raw/salaries.csv


This ensures that the pipeline can access the salary data during execution.

### Step 2 — Verify Repository Structure

The reproducibility of the workflow relies on maintaining the repository’s folder structure. The key directories are:

- `data/raw/` — raw inputs  
- `data/cleaned_data/` — OpenRefine cleaning histories  
...
### Step 3 — Review OpenRefine Provenance Files

All manual cleaning operations performed in OpenRefine are stored in JSON histories:
...
- Name normalization procedures  
- Removal of punctuation and whitespace inconsistencies  
- Salary formatting corrections  
- Standardization of column naming conventions  

Users may reapply these transformations in OpenRefine if desired, but this is not required for reproducing the final results because all processed outputs are generated automatically by the pipeline.

### Step 4 — Execute the Automated Pipeline (`run_all.ipynb`)
...
1. Loads raw performance and salary data from `data/raw/`.
2. Applies Python-based cleaning and standardization steps.
3. Merges the datasets using standardized player names.
4. Computes all derived metrics:
   - efficiency_index  
   - eff_per_min  
   - salary_million  
   - value_index  
5. Saves the merged dataset to:
data/merged_data/merged_salary_stats.csv

6. Saves the final feature-enhanced dataset to:
nba_salary_value_analysis/data/processed/final_dataset.csv

7. Generates all visualizations (8 figures total) and stores them in:
run_all/plots/


Upon completion, all artifacts needed to interpret, validate, or replicate the findings will be available in their respective directories.

### Step 5 — Accessing Output Files from Box

As required by Milestone 4, all output datasets and plots have also been uploaded to Box:

https://uofi.app.box.com/folder/354747066623


This Box folder includes:

- `final_dataset.csv`  
- `merged_salary_stats.csv`  
- All visualizations generated by the workflow  

This ensures that users and graders can retrieve outputs even without rerunning the notebook.

### Step 6 — Software Environment Requirements

All dependencies are listed in the `requirements.txt` file. The key libraries include:
...
Users should install dependencies via:
